...
func (wc *Context) IndexBlockWorkflow(ctx workflow.Context, in types.IndexBlockInput) error {
	...
	var skip bool
	if err := workflow.ExecuteActivity(ctx, wc.ActivityContext.PrepareIndexBlock, in).Get(ctx, &skip); err != nil {
		return err
	}
	if skip {
		return nil
	...
	if err := workflow.ExecuteActivity(ctx, wc.ActivityContext.IndexTransactions, in).Get(ctx, &indexedTxs); err != nil {
		return err
	}
	...
	if err := workflow.ExecuteActivity(ctx, wc.ActivityContext.IndexBlock, in).Get(ctx, &indexedHeight); err != nil {
		return err
	}

	// TODO: keep adding here more entities to index

	return workflow.ExecuteActivity(ctx, wc.ActivityContext.RecordIndexed, in).Get(ctx, nil)
}
