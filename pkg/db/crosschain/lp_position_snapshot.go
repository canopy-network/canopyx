...
func (db *DB) initLPPositionSnapshots(ctx context.Context) error {
	...
	// - source_chain_id: very low cardinality (few chains)
	// - pool_id: low cardinality (~10-100 pools per chain)
	// - address: high cardinality (thousands+ holders)
...
// When ActiveOnly=true, we use PREWHERE for is_position_active filter because:
// - is_position_active (UInt8, 1 byte) is much smaller than SELECT columns (12 columns)
// - Not in ORDER BY, so primary index doesn't help
// - High selectivity: many positions become inactive over time
...
func (db *DB) QueryLPPositionSnapshots(ctx context.Context, params LPPositionSnapshotQueryParams) ([]*indexermodels.LPPositionSnapshot, error) {
	...
	// PREWHERE is evaluated before WHERE, reading only this column first to filter rows
