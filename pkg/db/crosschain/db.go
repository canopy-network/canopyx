...
func (db *DB) createGlobalTable(ctx context.Context, cfg TableConfig) error {
	...
	// CRITICAL: Uses ON CLUSTER to ensure table exists on all replicas
	query := fmt.Sprintf(`
		...
		ENGINE = %s
		ORDER BY (%s)
	`, db.Name, globalTableName, db.OnCluster(), cfg.SchemaSQL, bloomFilter, clickhouse.ReplicatedEngine("ReplacingMergeTree", "height"), orderBy)
...
func (db *DB) createSyncMaterializedView(ctx context.Context, chainID uint64, chainDBName string, cfg TableConfig) error {
	...
	query := fmt.Sprintf(`
		...
		FROM "%s"."%s"
	`, chainDBName, mvName, db.OnCluster(), db.Name, globalTableName, columnList, chainDBName, sourceTableName)
...
func (db *DB) OptimizeTable(ctx context.Context, database, table string, final bool) error {
	return db.OptimizeTable(ctx, database, table, final)
}
