...
func (db *DB) initBlockSummaries(ctx context.Context) error {
	...
	// Create a production table with ReplacingMergeTree
	...
	stagingQuery := fmt.Sprintf(`
		...
		ORDER BY (height)
	`, db.Name, indexermodels.BlockSummariesStagingTableName, schemaSQL, clickhouse.ReplicatedEngine(clickhouse.ReplacingMergeTree, "height"))
