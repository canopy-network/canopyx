...
// This is a lightweight query that reads only 2 columns instead of 14.
// Use this when only the block time is needed (e.g., for date calculations).
func (db *DB) GetBlockTime(ctx context.Context, height uint64) (time.Time, error) {
	...
	err := db.QueryRow(ctx, query, height).Scan(&blockTime)
