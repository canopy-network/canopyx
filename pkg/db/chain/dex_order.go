...
func (db *DB) initDexOrders(ctx context.Context) error {
	queryTemplate := `
		CREATE TABLE IF NOT EXISTS "%s"."%s" (
			...
			success Bool,
			sold_amount UInt64 CODEC(Delta, ZSTD(3)),
			bought_amount UInt64 CODEC(Delta, ZSTD(3)),
			local_origin Bool,
			locked_height UInt64 CODEC(DoubleDelta, LZ4)
		) ENGINE = ReplacingMergeTree(height)
		ORDER BY (order_id, height)
	`

	// Create production table
	productionQuery := fmt.Sprintf(queryTemplate, db.Name, indexermodels.DexOrdersProductionTableName)
