...
func (db *AdminDB) initIndexProgress(ctx context.Context) error {
...
func (db *AdminDB) RecordIndexed(ctx context.Context, chainID uint64, height uint64, blockTime time.Time, indexingTimeMs float64, indexingDetail string) error {
	now := time.Now().UTC()
...
func (db *AdminDB) insertIndexProgress(ctx context.Context, ip *admin.IndexProgress) error {
	query := `
...
func (db *AdminDB) LastIndexed(ctx context.Context, chainID uint64) (uint64, error) {
...
// GetAllChainIndexProgress retrieves last indexed height for all chains.
// Uses aggregate table with fallback to base table.
func (db *AdminDB) GetAllChainIndexProgress(ctx context.Context) (map[string]uint64, error) {
