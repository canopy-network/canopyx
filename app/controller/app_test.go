...
func TestDesiredReplicasRespectsThresholds(t *testing.T) {
	app := &App{Logger: zaptest.NewLogger(t)}

	t.Run("returns min when below low watermark", func(t *testing.T) {
		ch := &Chain{ID: "chain-low", MinReplicas: 1, MaxReplicas: 5}
		stats := QueueStats{PendingWorkflowTasks: 3, PendingActivityTasks: 1}
	...
	t.Run("returns max when above high watermark", func(t *testing.T) {
		ch := &Chain{ID: "chain-high", MinReplicas: 2, MaxReplicas: 6}
		stats := QueueStats{PendingWorkflowTasks: backlogHighWatermark + 5, PendingActivityTasks: 10}
	...
	t.Run("interpolates between bounds", func(t *testing.T) {
		ch := &Chain{ID: "chain-mixed", MinReplicas: 1, MaxReplicas: 5}
		stats := QueueStats{PendingWorkflowTasks: 320, PendingActivityTasks: 90}
...
func TestDesiredReplicasHonoursCooldown(t *testing.T) {
	...
	ch := &Chain{
		...
		Hysteresis: ScaleState{
			...
			LastChangeTime:       time.Now().Add(-scaleCooldown / 2),
		},
	}
	stats := QueueStats{PendingWorkflowTasks: backlogHighWatermark + 50}

	require.Equal(t, int32(3), app.desiredReplicas(ch, stats), "should hold steady during cooldown")
	require.Equal(t, int32(3), ch.Hysteresis.LastDecisionReplicas)

	ch.Hysteresis.LastChangeTime = time.Now().Add(-2 * scaleCooldown)
