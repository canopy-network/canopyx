...
func (ac *Context) buildEventMaps(ctx context.Context, chainDb globalstore.Store, height uint64) (
        swapEvents, depositEvents, withdrawalEvents map[string]*globalstore.EventDexBatch, err error,
) {
    // Query events using lightweight query
    events, err := chainDb.GetDexBatchEvents(ctx, height)
    if err != nil {
        return nil, nil, nil, fmt.Errorf("query events at height %d: %w", height, err)
    }

    // Build event maps for O(1) lookups
    swapEvents = make(map[string]*globalstore.EventDexBatch)
    depositEvents = make(map[string]*globalstore.EventDexBatch)
    withdrawalEvents = make(map[string]*globalstore.EventDexBatch)
...
// insertDexBatchData inserts all DEX batch data to tables in PARALLEL
func (ac *Context) insertDexBatchData(
        ctx context.Context,
        chainDb globalstore.Store,
        orders []*indexermodels.DexOrder,
        deposits []*indexermodels.DexDeposit,
        withdrawals []*indexermodels.DexWithdrawal,
) error {
    ...
    if len(orders) > 0 {
        insertGroup.Submit(func() {
            ...
            ordersErr = chainDb.InsertDexOrders(insertCtx, orders)
        })
    }
    ...
    if len(deposits) > 0 {
        insertGroup.Submit(func() {
            ...
            depositsErr = chainDb.InsertDexDeposits(insertCtx, deposits)
        })
    }
    ...
    if len(withdrawals) > 0 {
        insertGroup.Submit(func() {
            ...
            withdrawalsErr = chainDb.InsertDexWithdrawals(insertCtx, withdrawals)
        })
    }
