...
- [ ] Emit metrics/logs for queue depth, scaling actions, and failures.
...
- [ ] Provide actions to trigger head/gap scans or request reindex of single height / height ranges.

## Reindex & Activity Updates
- [ ] Extend indexer workflows to accept a reindex flag and add a local activity guard that skips blocks when already up to date unless reindexing.
- [ ] Ensure reindex requests set distinct workflow IDs or pass metadata to avoid collision with existing runs.
- [ ] Add audit logging for manual reindex operations.

## UX & Feedback
- [ ] Add toast/notification system for admin actions (updates, reindex trigger, schedule commands).
- [ ] Include queue health indicators (green/yellow/red) based on thresholds aligned with scaling logic.

## Testing & Documentation
- [ ] Update Temporal/unit tests for new activity logic and controller scaling decisions.
- [ ] Add Playwright/Jest coverage for the updated admin UI flows (auth, chain edits, reindex trigger).
- [ ] Document new fields and admin workflows in `AGENTS.md` / README.
